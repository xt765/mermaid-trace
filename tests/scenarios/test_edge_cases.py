...
def test_recursive_calls(clean_logger):
    @trace
...
def test_large_arguments(clean_logger, caplog):
...
def test_unrepresentable_args(clean_logger, caplog):
...
def test_exception_in_handler(clean_logger):
...
def test_context_cleanup_on_error(clean_logger):
