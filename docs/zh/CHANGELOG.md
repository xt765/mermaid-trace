...
### 变更
- **依赖管理**: FastAPI 和 Uvicorn 现已成为必需依赖而非可选依赖。用户不再需要单独安装 `mermaid-trace[server]`。
- **CLI 改进**: 增强了帮助信息，添加了详细的描述、示例和功能亮点（全部使用英文）。
- **移除遗留代码**: 完全移除了已废弃的 `--master` 标志，使 CLI 界面更加简洁。

### 新增
- **离线支持**: Web 预览服务器现在使用本地静态资源（Tailwind CSS、Mermaid.js、svg-pan-zoom）而非外部 CDN 依赖。预览功能完全离线可用，无需网络连接。
- **版本命令**: 添加了 `mermaid-trace version` 命令用于显示已安装的版本。
- **浏览器控制**: 添加了 `--no-browser` 标志，用于在启动服务器时阻止自动打开浏览器。

### 修复
- **安装体验**: 解决了用户对需要单独安装可选依赖的困惑。现在 `pip install mermaid-trace` 即可提供所有功能。
