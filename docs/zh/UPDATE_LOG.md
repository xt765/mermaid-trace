...
## [0.7.1] - 2026-03-06 - 依赖管理优化与离线支持

### 核心变更
- **依赖管理优化**: 
  - FastAPI 和 Uvicorn 从可选依赖变为必需依赖。
  - 用户不再需要单独安装 `mermaid-trace[server]`，直接 `pip install mermaid-trace` 即可使用所有功能。
- **CLI 改进**: 
  - 增强了帮助信息，添加详细的描述、示例和功能亮点（全部使用英文）。
  - 新增 `mermaid-trace version` 命令，显示版本信息。
  - 新增 `--no-browser` 选项，控制是否自动打开浏览器。
  - 完全移除了已废弃的 `--master` 标志。

### 新增功能
- **离线支持**: 
  - Web 预览服务器现在使用本地静态资源（Tailwind CSS、Mermaid.js、svg-pan-zoom）。
  - 无需网络连接即可使用预览功能，完全离线可用。
  - 静态资源打包到 `src/mermaid_trace/static/` 目录。

### 文档更新
- 更新了所有文档以反映新的依赖结构和 CLI 变更。
- 同步更新了中英文文档。
